...
#include "../imagegenerator/GretagMacbethColorChecker.h"
...
bool ChromaticAdaptationTest3( )
{
   ...
   ChromaticAdaptationType adaptationType = CHROMATICADAPTATION_BRADFORD;
   ChromaticAdaptation ca( ColorConstants::sD93_XYZ, ColorConstants::sD50_XYZ, adaptationType);

   ca.ChromaticAdapt( pInImage );
   ImageIO::Write( pInImage, std::string("ChromaticD93-D50.ppm") );
   delete pInImage;

   pInImage = ImageIO::Read( std::string("../../../../stira/stira/testdata/chouffe.ppm") );

   ca.ChangeSourceWhite( ColorConstants::sD50_XYZ );
   ca.ChangeDestinationWhite( ColorConstants::sD75_XYZ );
   ca.ChromaticAdapt( pInImage );
   ImageIO::Write( pInImage, std::string("ChromaticD50-D75.ppm") );
...
bool RGB2HSV2RGBTest(Image* pInImage )
{
   ...
   double absMax = ImageStatistics::GetAbsMax( pDiff );
...
bool RGB2Lab2RGBTest(Image* pInImage )
{
   ...
   #ifdef FULL_DIAGNOSE_TEST
   ImageTools::Diagnose(pOut, string("Lab"));
   #endif
   pOut->LabTosRGB();
   Image* pDiff = ImageTools::CreateImageSSD( pInImage, pOut );
