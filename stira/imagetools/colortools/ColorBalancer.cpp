...
Image* ColorBalancer::RunGrayWorld(Image* pImageIn)
{
   if ( (pImageIn != 0) && (pImageIn->GetNumberOfBands() == 3) )
   {
      ...
      {
         meanPerBand[ bandNr ] = GridStatistics<double>::GetGridMean( pImageIn->GetBands()[ bandNr ] );
      }
      ...
      double mmin, mmax;
      ImageStatistics::GetMinMax( pOutImage, mmin, mmax );
...
Image* ColorBalancer::RunPerfectReflector(Image* pImageIn)
{
   if ( (pImageIn != 0) && (pImageIn->GetNumberOfBands() == 3) )
   {
      ...
      double mmax;
      int xMax=0, yMax=0;
      GridStatistics<double>::GetMax( pTmpImage->GetBands()[0], mmax, xMax, yMax );
