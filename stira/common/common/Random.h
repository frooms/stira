...
#ifndef STIRA_COMMON_RANDOMNUMBERS_H
#define STIRA_COMMON_RANDOMNUMBERS_H
...
//define getpid for ramdom seed
#if defined(__WIN32__) || defined(_WIN64)
   #include <ctime>
   inline unsigned getpid(void)
   {
      return static_cast<unsigned>(time(0));
   }
#else
...
private:
   ...
   // C++ does not allow initialization of non-integer static const data types in the header!!!
...
// Discrete Distributions
   ...
   //----------------------------------------------------------------------------
   // Pascal is under Windows a keyword that switches the order of parameters on the stack; therefore, name is changed to pascalDistribution
   int pascalDistribution( int s, double p )              // Pascal
   {                                          // successes s, probability p
...
// Number Theoretic Distributions
   ...
   void avoidance( double x[], int ndim )   // Maximal Avoidance (N-D)
   {
      ...
      if ( in == -1 ) {
         ...
         for ( k = 1; k <= MAXDIM; k++ ) {
            for (int jj = 1; jj <= (int)(mdeg[ k ]); jj++ ) u[ jj ][ k ] <<= ( MAXBIT - jj );
