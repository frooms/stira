...
public:
   ...
   /** \brief writes current magnitude image to disk
     * \param fileNameMagnitude name for file for magnitude image*/
   void ExportMagnitudeImage( std::string fileNameMagnitude );
   ...
   /** \brief writes current orientation image to disk
     * \param fileNameDirection name for file for orientation image
     * \param threshold threshold value for magnitude (below this, we just put black instead of coded orientation)
     * \param myMode color mapping scheme to use*/
   void ExportOrientationImage( std::string fileNameDirection, double threshold=3, ColorExportMode myMode=EXPORT_HSV_MODE);
