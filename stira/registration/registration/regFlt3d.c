...
/* FUNCTION: regFloat3d							*/
/************************************************************************/
int regFloat3d	(struct	rParam	*regDataPtr, struct fitRec *params)
{
		...
		for (l = regDataPtr->levels - 1; (l >= 0); l--) {
		  if ((l == (regDataPtr->levels - 1)) || (l >= (regDataPtr->lastLevel - 1))) {
		    printf("STATUS - Optimizing level %d \n", l);
			  ...
			  case Marquardt:
				if (optimize( &fit, &(regDataPtr->directives), ia, pyrHdl1[l], pyrHdl2[l],
				              mskHdl1[l], mskHdl2[l], outPtr, mskPtr, &lambda, regDataPtr->firstLambda,
                                              ...
                                              (int)nxRange[l], (int)nyRange[l], (int)nzRange[l]
                                            ) == ERROR) 
                                {
				  freePyramids(pyrHdl1, pyrHdl2, mskHdl1, mskHdl2, regDataPtr->levels);
		...
		freePyramids(pyrHdl1, pyrHdl2, mskHdl1, mskHdl2, regDataPtr->levels);
		params->dx[0]      = fit.dx[0];
		params->dx[1]      = fit.dx[1];
		params->skew[0][0] = fit.skew[0][0];
		params->skew[0][1] = fit.skew[0][1];
		params->skew[1][0] = fit.skew[1][0];
		params->skew[1][1] = fit.skew[1][1];
		return(!ERROR);
} /* End of regFloat3d */
